...
std::ifstream::pos_type Module::filelength(const char* filename) 
{
...
std::string	Module::MakeFullPath(std::string& p)
{
     std::string	str = p.substr(0, 0);
     if(str == "\"" || str == "\'" || str == "<"){
...
HRESULT	Module::ReadFile(std::string& filename)
{
     FILE* lpFP;
     ...
     dwMdlSize = filelength(FileName.c_str());
