...
/* 新dest1を返す */
{
	...
	UCHAR *s, *times_src0 = 0, *times_dest0 = 0, c, len, range;

	label0 = GOL_memmanalloc(&GOL_memman, nask_maxlabels * sizeof (struct STR_LABEL));
	subsect0 = GOL_memmanalloc(&GOL_memman, nask_maxlabels /* == MAXSUBSETCS */ * sizeof (struct STR_SUBSECTION));
	value = GOL_memmanalloc(&GOL_memman, sizeof (struct STR_VALUE));
	labelvalue = GOL_memmanalloc(&GOL_memman, sizeof (struct STR_VALUE));
...
//	min = max = 0; /* 次に生成するのは、variable0[l0 + lc] */
	...
	putvalue0:
				...
				dest0 += len;
				/* レンジチェックなどをやる */
				if (len == 1 && !(-128 <= value->min && value->min <= 255))
					*dest0++ = 0xe8;
				if (len == 2 && !(-0x8000 <= value->min && value->min <= 0xffff))
					*dest0++ = 0xe8;
				if (value->delta) {
					/* 不定値エラー */
					*dest0++ = 0xe9;
			...
			if (c <= 0x91) {
				...
				if ((src0[1] & 0x0f) == 0)				/* bugfix 2008.11.24 by K */
					vba->typlen = 0x30;
	...
	GOL_memmanfree(&GOL_memman, label0);
	GOL_memmanfree(&GOL_memman, subsect0);
	GOL_memmanfree(&GOL_memman, value);
	GOL_memmanfree(&GOL_memman, labelvalue);
