...
/* 新dest1を返す */
{
     ...
     label0 = reinterpret_cast<struct STR_LABEL *>(
	  GOL_memmanalloc(&GOL_memman, nask_maxlabels * sizeof (struct STR_LABEL))
	  );
     subsect0 = reinterpret_cast<struct STR_SUBSECTION *>(
	GOL_memmanalloc(&GOL_memman, nask_maxlabels /* == MAXSUBSETCS */ * sizeof (struct STR_SUBSECTION))
	  );
     
     value = reinterpret_cast<struct STR_VALUE *>(
	  GOL_memmanalloc(&GOL_memman, sizeof (struct STR_VALUE))
	  );
