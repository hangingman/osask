...
#define E_LABEL0		16
const int nask_L_LABEL0  = 16384;       /* externラベルは16300個程度使える */
const int nask_maxlabels = 64 * 1024;	/* 64K個(LL:88*64k) */
...
static void *cmalloc(int size)
{
     ...
     char *p = reinterpret_cast<char*>(
	  		GOL_memmanalloc(&GOL_memman, size)
	  	);
     for (i = 0; i < size; i++)
	  p[i] = 0;
     return p;
...
/* dest1を返す(NULLならあふれた) */
{
     int i, j, k, prefix_def = 0, tmret = 0;
     ...
     status = reinterpret_cast<struct STR_STATUS*>(
	  GOL_memmanalloc(&GOL_memman, sizeof (*status))
	  );

     decode = reinterpret_cast<struct STR_DECODE*>(
	  GOL_memmanalloc(&GOL_memman, sizeof (*decode))
	  );

     ifdef  = reinterpret_cast<struct STR_IFDEFBUF*>(
	  GOL_memmanalloc(&GOL_memman, sizeof (*ifdef))
	  );

     status->expression = reinterpret_cast<struct STR_TERM*>(
	  GOL_memmanalloc(&GOL_memman, EXPR_MAXLEN * sizeof (struct STR_TERM))
	  );
     ...
     ifdef->bp0 = reinterpret_cast<UCHAR*>(
	  GOL_memmanalloc(&GOL_memman, 256)
	  );

     ifdef->bp1 = ifdef->bp0 + 256;

     labelbuf0 = reinterpret_cast<UCHAR*>(
	  GOL_memmanalloc(&GOL_memman, nask_LABELBUFSIZ)
	  );
     labelbuf = labelbuf0;
     
     locallabelbuf0 = reinterpret_cast<UCHAR*>(
	  GOL_memmanalloc(&GOL_memman, 256)
	  );
     locallabelbuf = locallabelbuf0;
     
     for (i = 0; i < 9; i++) {
	  ifdef->expr[i] = reinterpret_cast<UCHAR*>(
	       GOL_memmanalloc(&GOL_memman, EXPR_MAXSIZ)
	       );
     }
     
     labelflags = reinterpret_cast<UCHAR*>(
	  GOL_memmanalloc(&GOL_memman, nask_maxlabels)
	  );
     ...
     section->total_len += dest0 - section->p;
     src = reinterpret_cast<UCHAR*>(
	  GOL_memmanalloc(&GOL_memman, i = dest0 - dest00)
	  );
     ...
     GOL_memmanfree(&GOL_memman, src1 - i);
...
overrun:
     GOL_memmanfree(&GOL_memman, decode);
     GOL_memmanfree(&GOL_memman, labelbuf0);
     GOL_memmanfree(&GOL_memman, locallabelbuf0);
     GOL_memmanfree(&GOL_memman, status->expression);
     GOL_memmanfree(&GOL_memman, status->mem_expr);
     GOL_memmanfree(&GOL_memman, status);
     GOL_memmanfree(&GOL_memman, ifdef->bp0);
     for (i = 0; i < 9; i++)
	  GOL_memmanfree(&GOL_memman, ifdef->expr[i]);
     GOL_memmanfree(&GOL_memman, ifdef);
     GOL_memmanfree(&GOL_memman, labelflags);
     GOL_memmanfree(&GOL_memman, sectable);
     return dest0;
}
...
/* listがあふれても続行 */
{
     int len, linecount = 0, srcl = 0, i, addr, secno, file_len = 0, g_symbols = 0, e_symbols = 0;
     struct STR_OUTPUT_SECTION *sectable = reinterpret_cast<struct STR_OUTPUT_SECTION*>(
	  GOL_memmanalloc(&GOL_memman, MAX_SECTIONS * sizeof (struct STR_OUTPUT_SECTION))
	  );
     UCHAR *srcp, *file_p = 0, *string0, *dest = dest0;
     UCHAR *lbuf0 = reinterpret_cast<UCHAR*>(GOL_memmanalloc(&GOL_memman, 1024)), *lbuf;
     UCHAR *ebuf0 = reinterpret_cast<UCHAR*>(GOL_memmanalloc(&GOL_memman, 32)), *ebuf; /* エラーバッファ */

     UCHAR c, status, adrflag = 0, cc, format = 0, file_aux = 0;
     ...
     do {
	  ...
	  if (srcp[0] == REM_8B) {
	       if (srcp[1] == 0) { /* file */
		    file_len = srcp[2];
		    file_p = reinterpret_cast<UCHAR*>(get4b(&srcp[4]));
	  ...
	  do {
	       ...
	       if (srcp[0] == REM_8B) {
		    if (1 <= srcp[1] && srcp[1] <= 2) {
			 len = srcp[2] | srcp[3] << 8;
			 lbuf = reinterpret_cast<UCHAR*>(get4b(&srcp[4]));
     ...
     for (;;) {
	  ...
	  if (c == 0xf7) {	/* switchで書いたら、lcc-win32が死んでしまった */
	       ...
	       srcl = get4b(&src0[1]);
	       srcp = reinterpret_cast<UCHAR*>(get4b(&src0[5]));
	       if (srcl) {
...
error:
     ...
     GOL_memmanfree(&GOL_memman, lbuf0);
     GOL_memmanfree(&GOL_memman, ebuf0);
     GOL_memmanfree(&GOL_memman, sectable);
     return dest;
}
