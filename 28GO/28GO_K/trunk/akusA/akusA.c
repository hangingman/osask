...
int aksa(UCHAR *p, UCHAR *p1, UCHAR *s, int eip, int dd, UCHAR *spbuf);

#define	CMDLIN_IN		0
#define CMDLIN_OFS		1
#define	CMDLIN_FLG_R	2
#define	CMDLIN_EIP		3
#define CMDLIN_D		4
#define CMDLIN_LIN		5
...
void G01Main()
{
	...
	int i, ofs, eip, dd, lin, j;

	g01_setcmdlin(cmdusg);
	sbuf = jg01_malloc(8 * 1024 * 1024 + 256 + 1024);
	spbuf = sbuf + 256;
	fbuf0 = spbuf + 1024;

	g01_getcmdlin_fopen_s_0_4(CMDLIN_IN);
	fbuf1 = fbuf0 + jg01_fread1f_4(8 * 1024 * 1024, fbuf0);
	ofs = g01_getcmdlin_int_s(CMDLIN_OFS);
	eip = g01_getcmdlin_int_s(CMDLIN_EIP);
	dd  = g01_getcmdlin_int_s(CMDLIN_D);
	lin = g01_getcmdlin_int_s(CMDLIN_LIN);
	if (g01_getcmdlin_flag_o(CMDLIN_FLG_R) != 0)
		eip += ofs;
	p = fbuf0 + ofs;
	if (ofs < 0 || p >= fbuf1)
		g01_putstr0_exit1("ofs out of range error!");
	g01_putstr0("file-ofs   EIP\n");
	do {
		i = aksa(p, fbuf1, sbuf, eip, dd, spbuf);
		if (i <= 0)
			break;
		sprintf(spbuf, "%08X %08X ", ofs, eip);
		g01_putstr0(spbuf);
		for (j = 0; j < 6; j++) {
			if (j < i) {
				sprintf(spbuf, "%02X ", p[j]);
				g01_putstr0(spbuf);
			} else
				g01_putstr0("   ");
		}
...
int aksa_ea(UCHAR *p, UCHAR *s, UCHAR len, UCHAR adrsiz, UCHAR segovr)
{
	...
	int i;
	UCHAR c = *p & 0xc7, displen, idx = 0, scl = 0, base;
	UCHAR *dp = &p[1];
...
op_group1_imm_w:
		...
		if (*p < 0x20) {
			...
			sprintf(spbuf, "0x0F 0x%02X -", bytes[1]);
			g01_putstr0_exit1(spbuf);
		}
		if ((*p & 0x07) == 0x06) {
			segovr = c;
op_pre:
