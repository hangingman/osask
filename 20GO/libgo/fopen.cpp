...
GO_FILE *GO_fopen(const char *filename, const char *mode)
{
	...
	filename = reinterpret_cast<const char*>(GOL_stepdir(&dir, (const UCHAR*)filename));
	gfp = GOL_open(dir, (const UCHAR*) filename);
	if (gfp == (GOL_FILE *) ~0) {
		errno = ENOENT;
		return NULL;
	}
	#warning "Fix me"
	//fp = reinterpret_cast<struct GO_FILE*>GOL_sysmalloc(sizeof (GO_FILE));
	fp->dummy = (intptr_t) gfp;
	fp->p0 = fp->p = gfp->p0;
